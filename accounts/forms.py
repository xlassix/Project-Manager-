...
def validate_email(value):
    if Users.check_email(email=value):
...
class Registration_Test_form(forms.Form):
    ...
    first_name = forms.CharField(validators=[ProhibitNullCharactersValidator],label='first name', max_length=30,widget=forms.TextInput(attrs={'class':"form-control",'placeholder':"First Name"}))
    last_name = forms.CharField(validators=[ProhibitNullCharactersValidator],label='last name', max_length=30, widget=forms.TextInput(attrs={'class':"form-control",'placeholder':"Last Name"}))
...
class Change_password_form(forms.Form):
    ...
    def get_data(self):
        ...
        return data
    def get_pass(self):
        data=super(Change_password_form, self).password
        return data

class Update_form(forms.Form):
    email = forms.EmailField(validators=[validate_email],label='email', max_length=50,widget=forms.TextInput(attrs={'class':"form-control",'placeholder':"Email",'value':'{{request.user.email}}'}))
    first_name = forms.CharField(validators=[ProhibitNullCharactersValidator],label='first name', max_length=30,widget=forms.TextInput(attrs={'class':"form-control",'placeholder':"First Name",'value':'{{request.user.first_name}}'}))
    last_name = forms.CharField(validators=[ProhibitNullCharactersValidator],label='last name', max_length=30, widget=forms.TextInput(attrs={'class':"form-control",'placeholder':"Last Name",'value':'{{request.user.last_name}}'}))
